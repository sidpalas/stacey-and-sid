...
module JekyllImport
  ...
  module WordpressDotCom
    ...
    def self.process(filename = {:source => "wordpress-no-comments.xml"})
      ...
      (doc/:channel/:item).each do |item|
        ...
        if status == "publish"
          ...
          published = false
          next # this throws away attachments, but that seems okay...
        end
        ...
        doc.css('img').each do |element|
          puts element
          src = element['src'] ? element['src'] : element['data-src']
          puts src
          u = URI src + '?format=2500w'
          if u.host.end_with? 'squarespace.com'  or u.host.end_with? 'squarespace-cdn.com'
            filename = u.path.sub(/^\//, '').gsub('/', '_') + '_' + (u.fragment || '')
            # filename too long if using the fully combined string from the url
            dest = ::File.join(@image_folder, filename.split(//).last(50).join)
        ...
        # pages shouldnt be dated...
        if type == 'post'
            name = "#{date.strftime('%Y-%m-%d')}-#{permalink_title}.html"
        else 
            name = "#{permalink_title}.html"
        end
